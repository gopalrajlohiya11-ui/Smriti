...
export default function PatientChatbot() {
  ...
  const [messages, setMessages] = useState(() => {
    ...
    const saved = sessionStorage.getItem(`smriti_chat_${targetId}`);
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    window.addEventListener('smriti_open_chat', handleOpenChat);
    return () => window.removeEventListener('smriti_open_chat', handleOpenChat);
  }, [isSpeakingAloud]);
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(`smriti_chat_${targetId}`, JSON.stringify(messages));
    ...
    setMessages([
      {
        ...
        sender: 'assistant',
        text: patientName 
          ? `Hello ${patientName}! I am Smriti, your caring companion. You can ask me about your medicines, today's schedule, doctor, or family anytime.`
          : `Hello! I am Smriti, your caring companion. You can ask me about your medicines, today's schedule, doctor, or family anytime.`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }
    ]);
  ...
  const isPatientRoute = location.pathname.startsWith('/patient') && location.pathname !== '/patient/login';
  const isLandingOrPublic = location.pathname === '/' || location.pathname === '';
  const shouldShow = isPatientLoggedIn || isPatientRoute || isLandingOrPublic;

  if (!shouldShow || location.pathname.startsWith('/caregiver') || location.pathname === '/patient/login') {
    return null;
  }
